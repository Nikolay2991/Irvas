...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let modalState = {};
  Object(_modules_changeModalState__WEBPACK_IMPORTED_MODULE_4__["default"])(modalState); //Модальные окна

  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])(); // Табы
...
const changeModalState = state => {
  const windowForm = document.querySelectorAll('.balcon_icons_img'),
        ...
        indowHeight = document.querySelectorAll('#height'),
  ...
  bindActionToElems('input', indowHeight, 'height');
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      ...
      postData('assets/server.php', formData).then(res => {
        ...
        // Отчистка инпутов
        clearInputs(); //отчистка данных формы (объекта modalState)
        ...
        emptyObject(state); // Удаление сообщения статуса через 5 секунд
...
__webpack_require__.r(__webpack_exports__);
const modals = e => {
  const bindModal = (btnSelector, modalSelector, closeSelector, closeClickOverlay = true) => {
  ...
  function calcScroll() {
    // создаем див для вычесления
    let div = document.createElement('div');
    div.style.width = '50px';
    div.style.height = '50px';
    div.style.overflowY = 'scroll';
    div.style.visibility = 'hidden';
    document.body.appendChild(div); //Вычисляем из ширины блока с скролом ширину без скрола

    let scrollWidth = div.offsetWidth - div.clientWidth; // удаляем див

    div.remove(); // Получаем результат (ширина скрола)
...
const timer = (id, deadline) => {
  // Функция добавления нуля если число однозначное
  const addZero = num => {
    if (num <= 9) {
      return '0' + num;
    } else {
  ...
  const getTimeRemaining = endtime => {
    // Записываем в разницу между обшим временем и текущим в милисекундах
    const t = Date.parse(endtime) - Date.parse(new Date()),
          // Далее переводим в секунды, минуты и часы
    seconds = Math.floor(t / 1000 % 60),
          minutes = Math.floor(t / (1000 * 60) % 60),
          hours = Math.floor(t / (1000 * 60 * 60) % 24),
          days = Math.floor(t / (1000 * 60 * 60 * 24)); // Вывод данных в виде объекта
  ...
  const setClock = (selector, endtime) => {
    const timer = document.querySelector(selector),
          ...
          // Обновляем функцию вывода времени каждую секунду
    timeInterval = setInterval(updateClock, 1000); //Вызываем функцию чтобы она сразу обновила время в таймере

    updateClock(); // Функция вывода времени
    ...
    function updateClock() {
      ...
      } // Выводим значение времени в верстку и добавляем 0 если надо функцией addZero
